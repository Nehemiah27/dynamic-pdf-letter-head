...
export interface IStorage {
  ...
  updateQuotation(
    id: number,
    updates: Partial<InsertQuotation>,
  ...
  updateQuotationItem(
    id: number,
    updates: Partial<InsertQuotationItem>,
  ...
  updateBranding(
    id: number,
    updates: Partial<InsertBranding>,
  ...
  updateInvoiceItem(
    id: number,
    updates: Partial<InsertInvoiceItem>,
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteUser(id: number): Promise<boolean> {
    ...
    await db
      ...
      .where(eq(userClientAssignments.userId, id));
    const result = await db.delete(users).where(eq(users.id, id)).returning();
    return result.length > 0;
  }

  async getUserClientAssignments(
    userId: number,
  ...
  async setUserClientAssignments(
    userId: number,
    clientIds: number[],
  ): Promise<void> {
  ...
  async deleteQuotationItemsByQuotationId(
    quotationId: number,
  ): Promise<boolean> {
    const result = await db
      .delete(quotationItems)
