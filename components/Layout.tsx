...
const Layout: React.FC<LayoutProps> = ({
  ...
  const isEdgeMode = dbStatus === "Disconnected";
  ...
  return (
    <div className="h-screen flex bg-white overflow-hidden font-['Inter']">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 z-30 shrink-0">
          ...
          <div className="flex-1 hidden lg:flex items-center gap-6">
            <div className="flex items-center gap-3">
              <div
                className={`w-2 h-2 rounded-full ${isEdgeMode ? "bg-amber-500" : "bg-emerald-500"} animate-pulse`}
              ></div>
              <h1 className="text-xs font-black text-[#2E3191] uppercase tracking-[0.2em]">
                Engineering Command Center
              </h1>
            </div>

            <div
              className={`flex items-center gap-4 px-4 py-2 rounded-xl border group cursor-default ${isEdgeMode ? "bg-amber-50 border-amber-100" : "bg-emerald-50 border-emerald-100"}`}
            >
              {isSyncing ? (
              ...
              <span className="text-[9px] font-black uppercase tracking-widest">
                System State:{" "}
                <span
                  className={isEdgeMode ? "text-amber-600" : "text-emerald-600"}
                >
                  {isEdgeMode ? "Edge Mode (Local)" : "Live Node"}
                </span>
              </span>
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-10 no-print bg-white h-full min-h-0">
          {children}
        </main>
      </div>
