...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    if (user && pass === "admin@123") {
      ...
      res
        .status(401)
        .json({ message: "Authorization Failure: Invalid credentials" });
    }
  } catch (error) {
    console.error("API Logic Exception:", error);
    res.status(503).json({
      message: "Service Temporarily Unavailable: Database Link Fault",
    });
  }
});
...
app.get("/api/data", async (req, res) => {
  try {
    const db = await connectToDatabase();
    const [users, clients, projects, quotations, branding, dbConfig] =
      await Promise.all([
    ...
    res
      .status(500)
      .json({ message: "Failed to synchronize with backend registry" });
  }
});
...
async function startServer() {
  try {
    ...
    console.warn(
      "[!] INFRASTRUCTURE WARNING: Real-time database unavailable. Seeding skipped.",
    );
    app.listen(PORT, () =>
      console.log(`[SYS] API Started in Edge Simulation Mode on Port ${PORT}`),
    );
  }
}
